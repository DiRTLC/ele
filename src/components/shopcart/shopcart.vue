...
<script>
  ...
  export default {
    ...
    mounted () {
      PubSub.subscribe('startDrop', (msg, startEl) => {
        const ball = this.balls.find(item => {
          return !item.isShow
        })
        if(ball) {
          ball.isShow = true
    ...
    methods: {
      beforeDrop (el) {
        ...
        let startEl = ball.startEl
        let {left, top} = startEl.getBoundingClientRect()
        let elLeft = 32
        let elBottom = 22
        console.log(elLeft, elBottom);
        el.style.transform = `translateY(${elBottom + top - window.innerHeight}px)`
        el.children[0].style.transform = `translateX(${left - elLeft}px)`
        el.ball = ball
      },
      drop (el) {
        Toast('提示信息');
      ...
      clear () {
        this.$store.dispatch('clearShopCart', {foodList: this.foodList})
      }
    },
    components: {
      cartcontrol
